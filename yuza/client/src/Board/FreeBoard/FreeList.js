...
function FreeList() {
    ...
    useEffect(() => {
        fetch('http://localhost:3001/free_board')
    ...
    const columns = [
        {
            ...
            align: 'center',
            renderCell: (params) => {
                console.log(params.row.isNotice);
                return params.row.isNotice ? <span style={{ color: 'red' }}>공지</span> : params.value;
            }
        },
        { field: 'free_title', headerName: '제목', width: 300, flex: 1, headerAlign: 'center' },
        { field: 'user_id', headerName: '글쓴이', width: 130, headerAlign: 'center', align: 'center' },
        {
            ...
            renderCell: (params) => params.value.substr(0,10)
        },
        { field: 'free_views', headerName: '조회수', width: 100, headerAlign: 'center', align: 'center' },
        { field: 'free_likes', headerName: '추천', width: 80, headerAlign: 'center', align: 'center' },
    ];
